...
<!-- - For each contract $i$ with predictors $\boldsymbol{x}_i$, follows a Poisson distribution: -->
\begin{align}
...
<!-- \begin{align} -->
<!-- \underset{\mu, \phi}{\text{minimize}} \left\{-\frac{1}{n} \sum_{i \in \mathcal{T}} y_i \ln[\mu(\boldsymbol{x_i})] - y_i\ln[\phi + \mu(\boldsymbol{x}_i)] + \phi\ln(\phi) -\phi\ln\left[\phi + \mu(\boldsymbol{x}_i)\right] + \ln[\Gamma(\phi + y_i)] - \ln[\Gamma(\phi)]\right\} -->
<!-- \end{align} -->

<!-- .ecriture-grise[There are 2 parameters to be estimated, we therefore need a 2-output neural network] -->

---
$\left(\frac{\phi}{\phi + \mu(\boldsymbol{x}_i)}\right)^\phi$ est incorrect
$\left(\frac{\mu(\boldsymbol{x}_i)}{\mu(\boldsymbol{x}_i) + \phi}\right)^\phi$ est correct <br>

$\text{Var}(Y_i) = \mu(\boldsymbol{x}_i) + \frac{\mu(\boldsymbol{x}_i)^2}{\phi}$
