...
\begin{align}
\underset{\mu}{\text{minimize}} \left\{ -\frac{1}{n} \sum_{i \in \mathcal{T}} y_i \ln\left[\mu(\boldsymbol{x_i})\right] - \mu(\boldsymbol{x}_i) - \ln(y_i!)\right\}
\end{align}
...
**Assumptions:**
- Given its predictors $\boldsymbol{x}_i$, the PMF for the number of claims $Y_i$ for contract $i$ is given by:
<!-- - For each contract $i$ with predictors $\boldsymbol{x}_i$, follows a Poisson distribution: -->
\begin{align}
P(Y_i = y_i) = \frac{e^{-\mu(\boldsymbol{x_i})}\mu(\boldsymbol{x_i})^{y_i}}{y_i!}, \quad y_i = 0, 1, \dots
\end{align}
